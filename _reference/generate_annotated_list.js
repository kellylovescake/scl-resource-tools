 ...
 * Annotated Bibliography — DOCX Generator v1
 *
 * Usage: node generate_annotated_list.js '<json>'
 * JSON shape: { title, ageGroup, preparedBy, books, digitalResources, outputPath }
 * Each book may include an optional `annotation` string (1 sentence, plain text).
 */
...
function buildFooter(preparedBy, dateStr, logoBytes) {
  ...
  const COL_MID    = convertInchesToTwip(2.0);   // page number column
  const COL_RIGHT  = CONTENT_W - COL_LOGO - COL_MID;  // prepared by column (~2.7")
  ...
  // Cell 3: "Prepared by: name" + date stacked, right-aligned
  ...
  return new TableRow({
    ...
    children: [
      new TableCell({
        ...
        margins: { top: 40, bottom: 80, left: 0, right: 120 },
        children: [
          // Title line
          new Paragraph({
            spacing: { after: 0 },
          ...
          // Annotation (1 sentence, indented, muted)
          ...(book.annotation ? [new Paragraph({
            spacing: { before: 40, after: 0 },
            indent: { left: convertInchesToTwip(0.2) },
            children: [new TextRun({
              text: book.annotation,
              italics: true, size: 17, color: "888888", font: "Calibri",
            })],
          })] : []),
        ],
      }),
      new TableCell({
        ...
        margins: { top: 40, bottom: 80, left: 120, right: 0 },
